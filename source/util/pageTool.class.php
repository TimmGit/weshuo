...
class pageTool
{
	...
	public function show($url)
	{
		...
		else 
		{
			...
			$html="<a href='".siteUrl($url.'/page/1')."'>$this->first</a>  <a href='".siteUrl($url.'/page/'.$page)."'>$this->prov</a>  ";
		}
		...
		else 
		{
			...
			$html.="<a href='".siteUrl($url.'/page/'.$page)."'>$this->next</a>  <a href='".siteUrl($url.'/page/'.$this->allPage)."'>$this->last</a> ";
		}
	...
	public function showNum($url,$num=5)
	{
		...
		if($this->nowPage>$num)
		{
			...
			$html.=" <a href='".siteUrl($url.'/page/'.$page)."'>".$this->up.$num.$this->pageName."</a> ";
		}
		if($this->allPage>1)
		{
			for ($i=0;$i<$num;$i++)
			{
				...
				{
					$html.=" <a href='".siteUrl($url.'/page/'.($this->nowPage-$num+$i))."'>".($this->nowPage-$num+$i)."</a> ";
				}
				else 
				{
					$html.=" <a href='".siteUrl($url.'/page/'.($this->nowPage+$i))."'>".($this->nowPage+$i)."</a> ";
				}			
			}
		}
		if($this->allPage>=$num)
		{
			$page=$this->nowPage+$num;
			$html.=" <a href='".siteUrl($url.'/page/'.$page)."'>".$this->down.$num.$this->pageName."</a> ";
		}
		if($this->nowPage+$num < $this->allPage)
		{
			$page=$this->nowPage+1;
